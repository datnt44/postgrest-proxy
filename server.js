...
const parsedOffset = parseInt(offsetRaw, 10) || 0;

const limit = Math.min(Math.max(parsedLimit, 1), 10000);
const offset = Math.max(parsedOffset, 0);

const result = await fetchFromPostgrest(tableName, limit, offset);
...
const result = await fetchFromPostgrest(TABLE_A2, 5, 0);
...
const result = await fetchFromPostgrest(TABLE_LENH_QUYETDINH, 5, 0);
